...
fun AddHomeProducts(
    ...
    var productExitsPrecie by remember { mutableStateOf(TextFieldValue("")) }
    var productExitsPrecieCarry by remember { mutableStateOf(TextFieldValue("")) }
    var productExitsPrecieInusual by remember { mutableStateOf(TextFieldValue("")) }
    var productExitsPrecieUsual by remember { mutableStateOf(TextFieldValue("")) }
    var productState by remember { mutableStateOf("N/P") }

    var isFormValid by remember { mutableStateOf(false) }
    var isProductDiferent by remember { mutableStateOf(false) }


    var showDialog by remember { mutableStateOf(false) }
    var showDifferentDialog by remember { mutableStateOf(false) }


    var desde by remember { mutableStateOf(TextFieldValue("")) }
    var hasta by remember { mutableStateOf(TextFieldValue("")) }
                    ...
                    )
                    {
                        ...
                        TextField(
                            ...
                            onValueChange = { newText ->
                                productCategory = newText
                                expanded = newText.text.isNotEmpty()

                                // Dividir el texto de búsqueda en palabras individuales y convertir todo a minúsculas
                                val searchWords = newText.text.split(" ").map { it.trim().lowercase() }

                                // Filtrar productos y quitar duplicados
                                filteredProducts = productStateHolder.productList
                                    .filter { product ->
                                        val category = product.categoria.lowercase()

                                        // Verificar si todas las palabras del texto de búsqueda están contenidas en la categoría
                                        searchWords.all { searchWord ->
                                            category.contains(searchWord)
                                        }
                                    }
                                    .distinctBy { it.categoria } // Eliminar duplicados basados en la categoría
                                    .sortedBy { it.categoria } // Ordenar alfabéticamente
                            },
                                    ...
                                    items(filteredProducts) { product ->
                                        Text(
                                            ...
                                            modifier = Modifier
                                                .fillMaxWidth()
                                                .clickable {
                                                    productCategory = TextFieldValue(product.categoria)
...
//                              ),
            ...
            confirmButton = {
                Button(
                    onClick = {
                        isProductDiferent = !productStateHolder.productList.any { it.nombre.equals(productName.text, ignoreCase = true) }
                        isFormValid = productName.text.isNotEmpty() &&
                                ...
                                productExitsPrecieCarry.text.isNotEmpty() &&
                        ...
                        if (isFormValid && isProductDiferent) {
                            ...
                            // Creación del nuevo producto con manejo de posibles nulos
                            val newProduct = Productos(
                                codigo = "${productStateHolder.productList.size + 1}", // Código automático
                                nombre = productName.text.replaceFirstChar { it.uppercase() }, // Se asegura de capitalizar el nombre
                                categoria = productCategory.text.replaceFirstChar { it.uppercase() }, // Capitaliza la categoría
                                comprasActuales = currentEntries ?: 0,
                                ventas = 0,
                                stock = currentEntries ?: 0,
                                precioCompra = productEntriesPrecie.text,
                                estado = productState,
                                desde = desde.text,
                                hasta = hasta.text,
                                precioSeleccionado = "0",
                                precioVenta = productExitsPrecie.text,
                                precioVentallevar = productExitsPrecieCarry.text,
                                precioVentaPorMayorClienteNormal = productExitsPrecieInusual.text,
                                precioVentaPorMayorClienteFrecuente = productExitsPrecieUsual.text
                            )

                            productStateHolder.addProduct(newProduct) // Llamar al método de añadir producto

                            coroutineScope.launch {
                                productRepository.guardarProducto(newProduct).collect {
    ...
    if (showDifferentDialog) {
        AlertDialog(

            onDismissRequest = { showDifferentDialog = false },
            title = {
                Text(text = "Advertencia")
            },
            text = {
                Text("Este producto ya se encuentra en la lista de inventario.")
            },
            confirmButton = {
                Button(
                    onClick = { showDifferentDialog = false }
                ) {
                    Text("OK")
                }
            }
